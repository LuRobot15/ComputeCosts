...
// YOU WILL ALSO USE THIS SECTION TO INITIALISE ANY VARAIABLES THAT REQUIRE PROPER INITIALISATION
	...
	int nODomestic = 0; // The number of domestic customers
	double QTotalDomesticPay; // the total aount that domestic customers paid that quarter
...
// NOW IT IS YOUR TURN, WRITE BELOW THE CODE FOR COMPUTING THE BILL AND UPDATING CUMULATIVE VALUES
	...
	if (UserChoice == 1) // processing and outputting bill
	{
		if (CustomerType == 1) // processing and outputting a domestic bill
		{
			// computing of charges to the customer who's data has been entered
			if (QWaterConsumption >= 1 && QWaterConsumption <= 5) // computing fresh waster charge for customers in band 1
				QFreshWaterCharge = (double) QWaterConsumption * 0.2;
			else
			{
				if (QWaterConsumption >= 6 && QWaterConsumption <= 12) // computing fresh waster charge for customers in band 2
					QFreshWaterCharge = (5 * 0.2) + (((double) QWaterConsumption - 5) * 0.35);
				else
				{
					if (QWaterConsumption >= 13 && QWaterConsumption <= 25) // computing fresh waster charge for customers in band 3
						QFreshWaterCharge = (5 * 0.2) + (7 * 0.35) + (((double) QWaterConsumption - 12) * 0.5);
					else
					{
						if (QWaterConsumption >= 26 && QWaterConsumption <= 40) // computing fresh waster charge for customers in band 4
					 		QFreshWaterCharge = (5 * 0.2) + (7 * 0.35) + (13 * 0.5) + (((double) QWaterConsumption - 25) * 0.75);
						else
						{
							if (QWaterConsumption > 40) // computing fresh waster charge for customers in band 5
								QFreshWaterCharge = (5 * 0.2) + (7 * 0.35) + (13 * 0.5) + (15 * 0.75) + (((double) QWaterConsumption - 40) * 2.5);
							else // if no fresh water was used by the customer
							{
								QFreshWaterCharge = 0;
							}
						}
			...
			QWasteWaterCharge = ((double) QWaterConsumption * 0.95); // computing the waste water charges
			QWasteWaterCharge = (double) QWasteWaterCharge * 0.25;
			QStandingCharges = 91 * 0.1; // computing standing charges
			QSurfaceWaterCharge = 10; // computing surface water charge
			QTotalCharge = QStandingCharges + QSurfaceWaterCharge + QWasteWaterCharge + QFreshWaterCharge; // computing the total charge to this customer for the company
			QVAT = 0.0; // sets VAT to 0 as domestic customers don't pay VAT
			QAmountToPay = QTotalCharge + QVAT; //computes the total that the customer needs to pay

			//cumulative charges after a domestic customers data has been entered
			QTotalDomesticPay = QTotalDomesticPay + QAmountToPay; // computes the total all domestic customers have paid this quarter
			nODomestic++; // increments the number of domestic customers
			QAverageDomesticBill = QTotalDomesticPay / nODomestic; // computes the adverage domestic bill
			if (QMaxDomesticBill < QAmountToPay) // changes the maximum domestic bill if neccessary
				QMaxDomesticBill = QAmountToPay;

			QTotalDomestic = QTotalDomestic + QWaterConsumption; // computes the total amount of fresh water supplied to domestic customers

		}
		else
		{
			if (CustomerType == 2) // processing a commercial bill
			{
				QFreshWaterCharge = QWaterConsumption * 2.5; // computes the fresh water charge for the customer
				QWasteWaterCharge = (QWaterConsumption * 0.95) * 2.0; // computes the waste water charge to the customer
				QSurfaceWaterCharge = 50; // computes surface water charge
				QStandingCharges = 1.3 * 91; // computes standing charges
				QTotalCharge = QStandingCharges + QSurfaceWaterCharge + QWasteWaterCharge + QFreshWaterCharge; // computing the total charge to this customer for the company
				QVAT = QTotalCharge * 0.2; // computes the VAT to the customer
				QAmountToPay = QTotalCharge + QVAT; //computs the total amount for the customer to pay
			}
		}

		// cumulative charges
		QTotalFreshWater = QTotalFreshWater + QWaterConsumption; // computes the total amount of fresh water used
		QRevenue = QRevenue + QTotalCharge; //computes the total revenue for the quarter
		QCost = QCost + (QWaterConsumption * 1); // computes the total cost for the company
		QProfit = QRevenue - QCost; // computes company profit
		QIncomeTax = QProfit * 0.25;
	}

// END End OF SECTION FOR WRITING YOUR CODE
// -------------------------------------------------------------------------------------------
